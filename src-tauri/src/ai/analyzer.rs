...
const SMALL_MODELS: &[(&str, &str)] = &[
    (
        "det_500m.onnx",
        "https://huggingface.co/public-data/insightface/resolve/main/models/buffalo_s/det_500m.onnx",
    ),
    (
        "w600k_mbf.onnx",
    ...
    (
        "genderage.onnx",
        "https://huggingface.co/public-data/insightface/resolve/main/models/buffalo_s/genderage.onnx",
    ),
];
...
fn download_model(url: &str, dest: &PathBuf) -> Result<(), String> {
...
pub fn ensure_models_downloaded(model_size: &ModelSize) -> Result<PathBuf, String> {
    let base = models_dir()?;
    let subdir = match model_size {
        ModelSize::Small => base.join("buffalo_s"),
        ModelSize::Large => base.join("buffalo_l"),
    };
...
impl FaceAnalyzerManager {
    pub async fn ensure_initialized(
        ...
        let dir = ensure_models_downloaded(&model_size)?;

        let det_path = dir.join("det_500m.onnx");
        let rec_path = dir.join("w600k_mbf.onnx");
        let attr_path = dir.join("genderage.onnx");
